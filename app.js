...
function ilGetReports(){ return ilLoad(IL_KEYS.reports, []).map(ilNormalizeReport); }
function ilGetIssues(){ return ilLoad(IL_KEYS.issues, []).map(i=>{ const x=Object.assign({},i); ['CreatedAt','UpdatedAt','ResolvedAt','ReopenedAt','TargetDate'].forEach(k=>{ if(x[k]!==null&&x[k]!==undefined) x[k]=ilTimestamp(x[k]); }); return x; }); }
...
function ilNormalizeReport(r){
  if(!r) return r;
  const x=Object.assign({},r);
  x.SubmittedAt=ilTimestamp(x.SubmittedAt);
  if(typeof x.PhotoURLs==='string'){
    try{ const parsed=JSON.parse(x.PhotoURLs); x.PhotoURLs=Array.isArray(parsed)?parsed:(x.PhotoURLs?[x.PhotoURLs]:[]); }catch(e){ x.PhotoURLs=x.PhotoURLs?[x.PhotoURLs]:[]; }
  }
  if(!Array.isArray(x.PhotoURLs)) x.PhotoURLs=[];
  return x;
}
...
function ilApplyCloudData(data, preserveLocal=true){
  ...
  if(Array.isArray(data.admins)){
    const localSettings=ilGetSettings();
    const localAdmins=Array.isArray(localSettings.admins)?localSettings.admins:[];
    const merged=ilMergeRecords(localAdmins,data.admins,'AdminID').map(a=>{
      const local=localAdmins.find(x=>x.AdminID===a.AdminID);
      // Cloud reads intentionally omit passwords; preserve a local password only for
      // the same admin so offline fallback login is not broken.
      return Object.assign({},a, local && local.Password ? {Password:local.Password}:{});
    });
    localSettings.admins=merged;
    ilSave(IL_KEYS.settings,localSettings);
  }
}
async function ilPullFromCloud(action='getAll'){
  if(!ilCloudEnabled()) return {ok:false, skipped:true};
  const data = await ilCloudGet(action);
  ilApplyCloudData(data,false);
  return data;
}
...
async function ilSyncResidents(){
  if(!ilCloudEnabled()) return {ok:false, skipped:true};
  // Push queued reports first, then perform one lightweight public-data pull.
  const pending=ilLoad(IL_PENDING_KEY,[]);
  for(const report of pending.slice()){
    try{ await ilPushReportToCloud(report); }
    catch(e){ console.warn('IssueLink pending report sync failed:',e); break; }
  }
  return await ilPullFromCloud('getPublicData');
}
...
function handlePhotoFile(file){
  ...
  reader.onload = e=>{
    ...
    img.onload = ()=>{
      ...
      if(dataUrl.length > 650000){ ilToast('Image is too large even after compression. Please choose a smaller photo.','error'); return; }
