...
function renderDashboard(){
  ...
  const recent = reports.slice().sort((a,b)=>ilTimestamp(b.SubmittedAt)-ilTimestamp(a.SubmittedAt)).slice(0,5);
...
function renderReports(){
  ...
  reports = reports.filter(r=>{
    ...
    if(kw && !((r.Title||'').toLowerCase().includes(kw) || String(r.ReportCode||'').toLowerCase().includes(kw))) return false;
    return true;
  }).sort((a,b)=>ilTimestamp(b.SubmittedAt)-ilTimestamp(a.SubmittedAt));
...
async function createIssueFromReport(reportId){
  ...
  if(ilGetSettings().storageMode==='Google Sheets Sync' && ilGetSettings().scriptUrl){
    try{
      await cloudMutation('saveIssue',{issue:ilGetIssues().find(i=>i.IssueID===issueId)});
      const timeline=ilGetUpdates().filter(u=>u.IssueID===issueId).slice(-1)[0];
      if(timeline) await cloudMutation('updateStatus',{update:timeline});
      await cloudMutation('saveReport',{report:ilGetReports().find(x=>x.ReportID===reportId)});
      ilToast('Issue created and synced.','success');
    }catch(e){ ilToast('Issue created locally, but cloud sync failed.','error'); }
  }else ilToast('Issue created successfully.','success');
  closeModal(); goSection('reports');
}
async function linkReportToIssue(reportId, issueId){
  ...
  if(ilGetSettings().storageMode==='Google Sheets Sync' && ilGetSettings().scriptUrl){
    try{
      await cloudMutation('linkReport',{reportId,issueId});
      await cloudMutation('saveIssue',{issue:ilGetIssues().find(i=>i.IssueID===issueId)});
      ilToast('Report linked and synced.','success');
    }catch(e){ ilToast('Report linked locally, but cloud sync failed.','error'); }
  }else ilToast('Report linked successfully.','success');
  closeModal(); goSection('reports');
}
...
async function saveIssueUpdate(issueId){
  ...
  if(ilGetSettings().storageMode==='Google Sheets Sync' && ilGetSettings().scriptUrl){
    try{
      const freshIssue=ilGetIssues().find(i=>i.IssueID===issueId);
      await cloudMutation('saveIssue',{issue:freshIssue});
      const timeline=ilGetUpdates().filter(u=>u.IssueID===issueId).slice(-1)[0];
      if(timeline) await cloudMutation('updateStatus',{update:timeline});
      ilToast('Status updated and synced.','success');
    }catch(e){ ilToast('Status updated locally, but cloud sync failed.','error'); }
  }else ilToast('Status updated successfully.','success');
...
async function saveDataSyncSettings(){
  ...
  if(settings.storageMode==='Google Sheets Sync' && settings.scriptUrl){
    try{
      await cloudMutation('saveSettings',{settings:Object.assign({Key:'appSettings',Value:JSON.stringify(settings)},settings)});
      ilToast('Settings saved and synced.','success');
    }catch(e){ ilToast('Settings saved locally, but cloud sync failed.','error'); }
  }else ilToast('Settings saved locally.','success');
  renderSettings();
}
...
async function syncNow(silent=false){
  ...
  if(!silent) ilToast('Refreshing cloud data…','info');
  try{
    // Normal refreshes are GET-only. The old implementation POSTed every local
    // record (including photo payloads), which made refreshes unnecessarily slow.
    let action='getDashboard';
    if(A.section==='reports') action='getReports';
    else if(A.section==='issues' || A.section==='linking' || A.section==='analytics') action='getAll';
    const result = await ilCloudGet(action);
    if(!result || result.ok===false) throw new Error(result?.error || 'The Apps Script bridge rejected the refresh.');

    ilApplyCloudData(result,false);
    const updated=ilGetSettings(); updated.lastSync=Date.now(); ilSave(IL_KEYS.settings,updated);

    // Re-render only what is needed instead of rebuilding every section.
    if(A.section==='dashboard') renderDashboard();
    else if(A.section==='reports') renderReports();
    else if(A.section==='issues') renderIssuesAdmin();
    else if(A.section==='linking') renderLinking();
    else if(A.section==='analytics') renderAnalytics();
    else if(A.section==='settings') renderSettings();
    if(!silent) ilToast('Cloud data refreshed successfully.','success');
    return true;
  }catch(err){
    console.error('IssueLink refresh error:',err);
    if(!silent) ilToast('Refresh failed. Check the Web App URL and Apps Script deployment.','error');
    return false;
  }finally{
...
async function cloudMutation(action,payload){
  const settings=ilGetSettings();
  if(settings.storageMode!=='Google Sheets Sync' || !settings.scriptUrl) return null;
  return await ilCloudPost(action,payload);
}
...
async function forceRefreshReports(){
  ...
  try{
    const settings=ilGetSettings();
    if(settings.storageMode==='Google Sheets Sync' && settings.scriptUrl){
      const result=await ilCloudGet('getReports');
      ilApplyCloudData(result,false);
      const st=ilGetSettings(); st.lastSync=Date.now(); ilSave(IL_KEYS.settings,st);
      renderReports();
      ilToast('Reports refreshed from Google Sheets.','success');
    }else{
      renderReports();
      ilToast('Reports refreshed from local data.','info');
    }
  }catch(err){
    console.error(err); ilToast('Could not refresh reports. Check your cloud connection.','error');
  }finally{
    if(btn){ btn.disabled=false; btn.innerHTML=btn.dataset.originalHtml || '<i class="fa-solid fa-arrows-rotate"></i> Refresh'; }
  }
}
...
async function toggleCategory(id){
  ...
  if(idx!==-1){ cats[idx].Active = cats[idx].Active===false ? true : false; ilSave(IL_KEYS.categories, cats); renderCategoriesAdmin(); if(ilGetSettings().storageMode==='Google Sheets Sync' && ilGetSettings().scriptUrl){ try{ await cloudMutation('saveCategory',{category:cats[idx]}); ilToast('Category updated and synced.','success'); }catch(e){ ilToast('Category changed locally, but cloud sync failed.','error'); } } }
}
...
async function toggleDepartment(id){
  ...
  if(idx!==-1){ deps[idx].Active = deps[idx].Active===false ? true : false; ilSave(IL_KEYS.departments, deps); renderDepartmentsAdmin(); if(ilGetSettings().storageMode==='Google Sheets Sync' && ilGetSettings().scriptUrl){ try{ await cloudMutation('saveDepartment',{department:deps[idx]}); ilToast('Department updated and synced.','success'); }catch(e){ ilToast('Department changed locally, but cloud sync failed.','error'); } } }
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  document.getElementById('reportsRefreshBtn')?.addEventListener('click', forceRefreshReports);
...
window.forceRefreshDashboard=forceRefreshDashboard; window.forceRefreshReports=forceRefreshReports;
